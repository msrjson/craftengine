...
class TestSeededLocales:
    """DEMO-ONLY: asserts the contents of `database/seeders/TranslationSeeder.py`,
    the sample application's own catalog — not engine behaviour. The engine side
    (four locales resolving independently, `pt` and `pt-BR` distinct, the
    fallback chain) is covered by `TestTranslationLookup` above, which seeds its
    own rows. Delete this class with the sample application's seeders.
    """
